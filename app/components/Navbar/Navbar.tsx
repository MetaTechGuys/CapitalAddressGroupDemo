  ...
  const menuItems = [
    {
      ...
      dropdown: [
        ...
        { label: t('navbar.dropdown.home.featured_projects'), href: '/featured' },
        { label: t('navbar.dropdown.home.testimonials'), href: '/testimonials' },
      ]
    },
    {
      id: 'about',
    ...
    },
    {
      ...
      href: '/services',
      dropdown: [
        { label: t('navbar.dropdown.services.real_estate'), href: '/services/real-estate' },
        { label: t('navbar.dropdown.services.property_management'), href: '/services/property-management' },
        { label: t('navbar.dropdown.services.investment_advisory'), href: '/services/investment' },
        { label: t('navbar.dropdown.services.consulting'), href: '/services/consulting' },
        { label: t('navbar.dropdown.services.valuation'), href: '/services/valuation' },
      ]
    },
    {
      id: 'contact',
      ...
      dropdown: [
        ...
        { label: t('navbar.dropdown.contact.office_locations'), href: '/contact/locations' },
        { label: t('navbar.dropdown.contact.support'), href: '/contact/support' },
        { label: t('navbar.dropdown.contact.partnership'), href: '/contact/partnership' },
        { label: t('navbar.dropdown.contact.schedule_meeting'), href: '/contact/meeting' },
      ]
    }
  ];
    ...
    <nav className={`navbar ${isScrolled ? 'scrolled' : ''} ${isDarkTheme ? 'dark' : 'light'}`}>
      <div className="navbar__container">
        ...
        <ul className={`navbar__menu ${isMobileMenuOpen ? 'active' : ''}`}>
          {menuItems.map((item) => (
            ...
            >
              ...
              >
                {item.label}
                <svg width="12" height="12" viewBox="0 0 24 24" className="navbar__svg navbar__dropdown-arrow">
        ...
        <div className={`navbar__search ${isSearchOpen ? 'active' : ''}`}>
          <form onSubmit={handleSearch} className="navbar__search-form">
            ...
            <button type="submit" className="navbar__search-submit">
              <svg width="16" height="16" viewBox="0 0 24 24" className="navbar__svg">
        ...
        <div className="navbar__actions">
          {/* Search Button */}
          <button 
            className={`navbar__action-btn navbar__search-btn ${isSearchOpen ? 'active' : ''}`}
            onClick={toggleSearch}
            aria-label={t('navbar.toggle_search')}
          >
            <svg width="20" height="20" viewBox="0 0 24 24" className="navbar__svg">
              <path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
            </svg>
          </button>
          ...
          <div className="navbar__language-dropdown">
            <button 
              className={`navbar__action-btn navbar__language-btn ${isLanguageDropdownOpen ? 'active' : ''}`}
            ...
            <div className={`navbar__language-menu ${isLanguageDropdownOpen ? 'active' : ''}`}>
              {languages.map((language) => (
                <button
                  key={language.code}
                  className={`navbar__language-option ${currentLanguage === language.code ? 'selected' : ''}`}
                  onClick={() => selectLanguage(language.code)}
                >
                                    <span className="navbar__language-name">{language.name}</span>
                  <span className="navbar__language-code">({language.code.toUpperCase()})</span>
                </button>
              ))}
            </div>
          </div>
