...
export default function Services() {
  ...
  const goToSection = useCallback((index: number) => {
    const sectionsLength = 4; // We have 4 sections: companies, cover, company-details, footer
    ...
    if (isMobile) {
      ...
      const sectionIds = ['companies', 'cover', 'company-details', 'footer'];
  ...
  const handleCompanyClick = useCallback((companyId: number) => {
    ...
    // Go to company-details section (index 2)
    setTimeout(() => {
      goToSection(2);
    }, 100);
  }, [goToSection]);
  ...
  const handleBackToCompanies = useCallback(() => {
    // Go to companies section (index 0)
    goToSection(0);
  }, [goToSection]);
  ...
  const sections = useMemo(() => [
    ...
    },
    {
      ...
      name: t('companyDetails.title') || 'Company Details'
    },
    { 
      id: 'footer', 
      component: <Footer />, 
  ...
  ], [t, handleCompanyClick, activeCompanyId, handleBackToCompanies]);
