...
import ProjectsCover from "../components/ProjectsCover/ProjectsCover";
...
export default function ProjectsPage() {
  ...
  const goToSection = useCallback((index: number) => {
    const sectionsLength = 4; // We have 4 sections: projects, projects-cover, project-details, footer
    
    if (index < 0 || index >= sectionsLength || index === currentSection || isTransitioning) {
      return;
    }
    ...
    if (isMobile) {
      ...
      const sectionIds = ['projects', 'projects-cover', 'project-details', 'footer'];
  ...
  const handleProjectClick = useCallback((projectId: number) => {
    ...
    // Go to project-details section (index 2)
    setTimeout(() => {
      goToSection(2);
    }, 100);
  }, [goToSection]);
  ...
  const handleBackToProjects = useCallback(() => {
    // Go to projects section (index 0)
    goToSection(0);
  }, [goToSection]);
  ...
  const sections = useMemo(() => [
    ...
    },
    {
      ...
      name: t('projectDetails.title') || 'Project Details'
    },
    { 
      id: 'footer', 
      component: <Footer />, 
  ...
  ], [t, handleProjectClick, activeProjectId, handleBackToProjects]);
